...
public class AgentLoader {
    ...
    public static String extractJar(URL path, String agentName) {
        ...
        try {
            String[] jarNames;
            if (isWindows()) {
                jarNames = path.getPath().split(":/", 2);
            } else {
                jarNames = path.getPath().split(":");
            }
            String[] packageFileAndFileOffset = jarNames[1].split("!/");

            // the full path of Final Agent Jar File
            fullPath = System.getProperty("java.io.tmpdir") + File.separator + agentName + ".jar";
    ...
    public static boolean isSolarisSystem() {
        String osName = System.getProperty("os.name");
        return osName.toLowerCase().startsWith("solaris");
    }
